...
def vehicle_passenger_by_minute_dataframe(route_df: pd.DataFrame, vehicles: list[VehicleState]) -> pd.DataFrame:
    """각 vehicle_id별로 매 분 현재 탑승 중인 승객 수를 출력한다."""
    columns = ["vehicle_id", "time_min", "time_hhmm", "onboard_passengers"]
    if not vehicles:
        return pd.DataFrame(columns=columns)

    output_end = end_time_minutes()
    if not route_df.empty:
        output_end = max(
            output_end,
            int(pd.to_numeric(route_df["arrival_time"], errors="raise").max()),
            int(pd.to_numeric(route_df["departure_time"], errors="raise").max()),
        )
    output_end = max(output_end, max(int(v.available_time) for v in vehicles))
    ...
    for state in vehicles:
        ...
        if route_df.empty:
            ...
            service_events = (
                route_df[
                    (route_df["vehicle_id"] == state.vehicle_id)
                    & (route_df["event"].isin(["Pickup", "Delivery"]))
                ]
                .sort_values(["arrival_time", "sequence"])
                .reset_index(drop=True)
            )

        event_pos = 0
        for minute in range(0, output_end + 1):
            while event_pos < len(service_events) and int(service_events.iloc[event_pos]["arrival_time"]) <= minute:
                onboard = int(service_events.iloc[event_pos]["onboard_passengers"])
                event_pos += 1
...
def main() -> None:
    ...
    save_csv(vehicle_passenger_by_minute_dataframe(route_df, vehicles), "vehicle_passenger_by_minute.csv")
